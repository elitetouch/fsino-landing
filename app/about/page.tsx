...
import {
  Bird, Cpu, LineChart, ShieldCheck, Sparkles, Thermometer,
} from 'lucide-react';

export const metadata: Metadata = {
  title: 'About',
  description:
    'Farm Support Innovation builds hardware and software for African smallholder and mid-scale farms. Formerly Farmspeak Technology.',
};

/**
 * About page — the mission, the products in plain language, the
 * "why now" for investors and grant funders, and the honest
 * registration line.
 *
 * Copy mirrors the tenant portal's About page so a farmer reading
 * either surface gets the same story.
 */
      ...
      <section className="border-b border-[var(--color-brand-border)] bg-gradient-to-b from-[var(--color-brand-accent)]/40 to-white">
        <div className="mx-auto max-w-[1200px] px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--color-brand-primary-deep)]">
            About Farm Support Innovation
          </p>
          <h1 className="mt-3 max-w-3xl text-[32px] font-bold leading-tight tracking-tight text-[var(--color-brand-fg)] sm:text-[44px]">
            An operating system for African smallholder farms.
          </h1>
          <p className="mt-4 max-w-2xl text-[15.5px] leading-relaxed text-[var(--color-brand-fg-soft)]">
            Real-time climate, breed-standard benchmarks, and bank-grade
            reporting for the farmers who feed a continent.
          </p>
        </div>
      </section>

      <section className="mx-auto max-w-[900px] space-y-8 px-4 py-16 sm:px-6 lg:px-8">
        <div className="rounded-2xl border border-[var(--color-brand-border)] bg-gradient-to-br from-[var(--color-brand-accent)] to-white p-6 sm:p-8">
          <p className="text-[15px] leading-relaxed text-[var(--color-brand-fg)]">
            Over 60% of Africa&apos;s protein comes from smallholder
            poultry farmers running flocks of five hundred to twenty
            thousand birds. They compete with commercial operations that
            use continuous climate monitoring, breed-standard
            benchmarking, structured record-keeping and bank-grade
            reporting. Tools priced for multi-million-dollar operations,
            not the family compound in Nigeria.
          </p>
            ...
            <div>
              ...
              <ul className="mt-3 space-y-2 text-[13.5px] leading-relaxed text-[var(--color-brand-fg-soft)]">
                <li>
                  ...
                  See how this cycle&apos;s feed use and bird weight is
                  doing against your own past cycles and against other
                  farms raising the same breed in the same weather.
                </li>
                <li>
                  <strong className="text-[var(--color-brand-fg)]">Cost and profit projection.</strong>{' '}
                  Show the total spend so far and what the final profit
                  is likely to be, so a farmer knows on day 15 whether
                  this batch will make money.
                </li>
                <li>
                  <strong className="text-[var(--color-brand-fg)]">Best day to sell.</strong>{' '}
                  Look at how fast the birds are growing and tell the
                  farmer the best day to send them to market, days
                  before they would guess by eye.
                </li>
                <li>
                  <strong className="text-[var(--color-brand-fg)]">A system that learns.</strong>{' '}
                  Every alert now asks the farmer if it was &quot;helpful&quot; or
                  a &quot;false alarm&quot;. Once enough farmers have answered,
                  the system learns to catch real problems earlier and
                  stop sending alerts that turn out to be nothing.
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Stat label="Farms served" value="5,000+" hint="Across Nigeria, Ghana and Cameroon" />
          <Stat label="Pens monitored" value="2,800+" hint="Live climate telemetry, 24/7" />
          <Stat label="Records logged" value="220k+" hint="Feed, mortality, vaccine, sales" />
        </div>
        ...
        <div className="rounded-2xl border border-[var(--color-brand-border)] bg-white p-6 sm:p-8">
          ...
          <h2 className="mt-2 text-[20px] font-bold tracking-tight text-[var(--color-brand-fg)]">
          ...
          <p className="mt-3 text-[14px] leading-relaxed text-[var(--color-brand-fg-soft)]">
            ...
            farmers with fewer than 20,000 head. Nothing built for that
            scale exists at that price. We build both the sensor that
            sees inside the pen and the app that turns the reading into
            a decision. A competitor would have to build both to catch
            up.
          </p>
          <p className="mt-3 text-[14px] leading-relaxed text-[var(--color-brand-fg-soft)]">
            Every cycle you log teaches the system a little more. Every
            alert you mark helpful or false alarm makes it smarter. The
            longer a farm uses us, the more accurate the advice becomes
            for that farm&apos;s breed, climate and way of working. That
            advantage grows every year, and it would take a new
            competitor years to catch up.
          </p>
          <p className="mt-4 text-[13px] italic text-[var(--color-brand-muted)]">
            Talk to us:{' '}
            <a
              href="mailto:hello@fsinnovation.net"
              className="font-bold text-[var(--color-brand-primary-deep)] hover:underline"
            >
...
function Stat({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <div className="rounded-2xl border border-[var(--color-brand-border)] bg-white p-5 text-center">
      ...
      <p className="mt-1 text-[11.5px] text-[var(--color-brand-muted)]">{hint}</p>
    </div>
  );
}
